...
var FormValidation =
  ...
  (function() {
    ...
    _createClass(FormValidation, [
      ...
      {
        key: "_pushError",
        value: function _pushError(element, key, message) {
        ...
        value: function validate(callback) {
          ...
          for (var key in this.rules) {
            ...
            if (this.rules.hasOwnProperty(key)) {
              // trim first
              if (
                !this._isFound(this.rules[key]["trim"]) ||
                this.rules[key]["trim"]
              ) {
              ...
              // required
              if (
                this._isFound(this.rules[key]["required"]) &&
                this.rules[key]["required"]
              ) {
                if (document.getElementById(key).value.length === 0) {
                  this._pushError(
                    this.rules[key]["message"],
                    key,
              ...
              // minLength
              if (
                this._isFound(this.rules[key]["minLength"]) &&
              ...
              // maxLength
              if (
                this._isFound(this.rules[key]["maxLength"]) &&
                this.rules[key]["maxLength"]
              ) {
                ...
                if (document.getElementById(key).value.length > maxchar) {
                  this._pushError(
                    this.rules[key]["message"],
                    key,
                    "This field must be not more than " +
                      maxchar +
                      " characters long!"
                  );
                }
                  ...
                  if (this._isFound(this.rules[key]["method"])) {
                    ...
                    if (fn(document.getElementById(key)) === false) {
                      this._pushError(
                        this.rules[key]["message"],
                        key,
                        "This format field is not valid!"
                      );
                    }
                  }
                }
              ...
              // errorPlace
              if (
                this._isFound(this.rules[key]["errorPlace"]) &&
                this.rules[key]["errorPlace"]
              ) {
